...
import { AIProfile, UserProfile, ChatMessage, GalleryItem, JournalEntry, Memory, KnowledgeBaseDocument, ChatSession } from '../types';
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const loadData = async () => {
        try {
            ...
            aiCanGenerateImages: loadedProfile.aiCanGenerateImages ?? false,
                ...
                // Handle sessions for the active persona
                if (savedData.aiProfile) {
                    const profile = savedData.aiProfile;
                    if (profile.sessions && profile.sessions.length > 0) {
                        setSessions(profile.sessions);
                        const activeId = profile.activeSessionId || profile.sessions[0].id;
                        setActiveSessionId(activeId);
                        const activeSession = profile.sessions.find(s => s.id === activeId);
                        if (activeSession) {
                            setChatHistory(activeSession.messages);
                        }
                    } else {
                        // Create initial session from legacy chatHistory
                        const initialSession: ChatSession = {
                            id: 'initial-session',
                            title: 'Initial Chat',
                            messages: savedData.chatHistory || [],
                            createdAt: Date.now(),
                            updatedAt: Date.now(),
                        };
  ...
  const saveData = useCallback(async () => {
    ...
    try {
      // Sync active data into savedPersonas to ensure isolation
      const updatedSavedPersonas = savedPersonas.map(p => 
        p.id === aiProfile.id 
          ? { ...p, chatHistory, memories, journal } 
          : p
      );

      const data: AppState = {
          aiProfile: { ...aiProfile, chatHistory, memories, journal },
          savedPersonas: updatedSavedPersonas,
          userProfile,
          chatHistory,
          gallery,
          journal,
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(async () => {
        try {
            ...
            // We no longer store this in localStorage as it easily exceeds the 5MB quota with multiple personas.
        } catch (e) {
  ...
  const loadPersona = (id: string) => {
    ...
    setSavedPersonas(prev => prev.map(p => 
      p.id === aiProfile.id 
        ? { ...p, chatHistory, memories, journal, sessions, activeSessionId } 
        : p
    ));
  ...
  const updateChatMessage = (id: string, newContent: string) => {
    setChatHistory(prev => {
        const newHistory = prev.map(m => m.id === id ? { ...m, content: newContent } : m);
        // Update active session
        setSessions(sPrev => sPrev.map(s => 
            s.id === activeSessionId 
                ? { ...s, messages: newHistory, updatedAt: Date.now() } 
  ...
  const createNewSession = (title: string = 'New Chat') => {
    ...
    setChatHistory([]);
    addToast({ title: "New Chat Started", message: "A fresh conversation has been created.", type: "success" });
  };
  ...
  const deleteSession = (sessionId: string) => {
    if (sessions.length <= 1) {
        addToast({ title: "Cannot Delete", message: "You must have at least one chat session.", type: "warning" });
        return;
    }
    ...
    addToast({ title: "Chat Deleted", message: "The conversation has been removed.", type: "info" });
  };
